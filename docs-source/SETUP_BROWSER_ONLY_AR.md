...
> ⚠️ **سبب الشاشة البيضاء الأكثر شيوعًا:** لو ظل **Folder** على `/ (root)` بدلًا من `/docs`،
> سينشر GitHub ملفات جذر المستودع (ملف `index.html` المصدري الذي يشير إلى `/src/main.tsx`
> و`%BASE_URL%config.js`)، فلا تعمل الواجهة وتظهر **شاشة بيضاء فارغة**. تأكد أن الحقل
> **Folder = `/docs`** ثم احفظ وانتظر دقيقة حتى ينتهي بناء Pages (منبثقة خضراء بجوار Save).
